...
import { AnimatedThemeToggler } from "@/components/ui/animated-theme-toggler"; // Import Theme Switcher
...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    <div className={`card-nav-container absolute left-1/2 -translate-x-1/2 w-[90%] max-w-[800px] z-[99] top-[1.2em] md:top-[2em] ${className}`}>
      <nav
        ref={navRef}
        className={`card-nav ${isExpanded ? 'open' : ''} block h-[60px] p-0 rounded-xl shadow-2xl border border-[hsl(var(--border))] relative overflow-hidden will-change-[height]`}
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
          ...
          <div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 order-1 md:order-none">
            <img src={logo} alt={logoAlt} className="logo h-[50px] w-auto" />
          </div>

          {/* Right Side: Theme Switcher + CTA */}
          <div className="flex items-center gap-3 h-full">
            <div className='hidden md:block'>
                <AnimatedThemeToggler className="text-current hover:text-[hsl(var(--primary))] transition-colors" />
            </div>
        ...
        >
          {/* Mobile Theme Toggler (Visible only inside menu on small screens) */}
          ...
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="nav-card select-none relative flex flex-col gap-2 p-[12px_16px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%] border border-[hsl(var(--border))]/10"
              ref={setCardRef(idx)}
