...
export async function POST() {
  try {
    ...
    if (existingTasks.empty) {
      for (const task of [...SAMPLE_DAILY_TASKS, ...SAMPLE_SPECIAL_TASKS]) {
        await addDoc(tasksRef, task)
        results.tasks.added++
      }
    } else {
      results.tasks.skipped = existingTasks.size
    }
    ...
    if (existingShop.empty) {
      for (const item of SAMPLE_SHOP_ITEMS) {
        ...
        results.shopItems.added++
      }
    } else {
      results.shopItems.skipped = existingShop.size
    }
