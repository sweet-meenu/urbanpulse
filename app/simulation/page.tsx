  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-12">
      <div className="max-w-7xl mx-auto">
        <Link href="/simulations" className="inline-flex items-center gap-2 text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))] transition-colors mb-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12">
          <div>
            <h1 className="text-5xl md:text-7xl font-bold font-space mb-4">Digital Twin <span className="text-[hsl(var(--secondary))]">Simulation</span></h1>
            <p className="text-xl text-[hsl(var(--muted-foreground))] max-w-3xl">Real-time analysis with data from Open-Meteo, TomTom, and environmental monitoring systems.</p>
          </div>

          <Button onClick={handleRefresh} disabled={isRefreshing} variant="outline" className="flex items-center gap-2 whitespace-nowrap bg-transparent">
            {isRefreshing ? <IconLoader size={20} className="animate-spin" /> : <IconRefresh size={20} />} Refresh Latest
          </Button>
        </div>

        {error && <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm mb-8">{error}</div>}

        {!TOMTOM_KEY && <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-700 text-sm mb-8">TomTom API key not configured — traffic insights are disabled. Set NEXT_PUBLIC_TOMTOM_API_KEY in your environment to enable.</div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <Card className="p-8 space-y-4">
            ...
            <span className="inline-block px-3 py-1 text-xs font-medium rounded-full bg-[hsl(var(--primary))]/10 text-[hsl(var(--primary))]">Active</span>
          </Card>
          ...
          <Card className="p-8 space-y-4">
            ...
            <span className="inline-block px-3 py-1 text-xs font-medium rounded-full bg-[hsl(var(--secondary))]/10 text-[hsl(var(--secondary))]">Version 3.1</span>
          </Card>
          ...
          <Card className="p-8 space-y-4">
            ...
            <span className="inline-block px-3 py-1 text-xs font-medium rounded-full bg-purple-500/10 text-purple-400">Processing</span>
          </Card>
        </div>

        {loading ? (
          <Card className="p-12 text-center"><IconLoader size={48} className="animate-spin mx-auto mb-4 text-[hsl(var(--primary))]" /><p className="text-lg text-[hsl(var(--muted-foreground))]">Loading data...</p></Card>
        ) : tomtomFlow ? (
              ...
              <div className="flex flex-col lg:flex-row gap-6 items-start">
                <div className="flex-1">
                  <div className="flex gap-8 items-center">
                    <div>
                      <p className="text-sm text-[hsl(var(--muted-foreground))]">Current Speed</p>
                      <div className="text-5xl font-bold">{Number((((tomtomFlow as unknown) as Record<string, unknown>)['currentSpeed'] as number) ?? 0) || 0}<span className="text-lg font-normal"> km/h</span></div>
                    </div>
                    <div>
                      <p className="text-sm text-[hsl(var(--muted-foreground))]">Free Flow</p>
                      <div className="text-5xl font-bold">{Number((((tomtomFlow as unknown) as Record<string, unknown>)['freeFlowSpeed'] as number) ?? 0) || 0}<span className="text-lg font-normal"> km/h</span></div>
                    </div>
                  </div>
                      ...
                      return (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <div className="text-sm text-[hsl(var(--muted-foreground))]">{ratio >= 0.95 ? 'Smooth Flow' : ratio >= 0.7 ? 'Moderate' : 'Congested'}</div>
                            <div className="text-sm font-medium">{pct.toFixed(0)}%</div>
                          </div>
                          <div className="w-full bg-[hsl(var(--border))] h-3 rounded-full overflow-hidden"><div className="h-3 rounded-full bg-green-500" style={{ width: `${pct}%` }} /></div>
                        </div>
                      )
                    })()}
                  </div>
                ...
                <div className="w-48">
                  <div className="rounded-xl p-4 bg-[hsl(var(--card))] border border-[hsl(var(--border))] text-center">
                    <div className="text-sm text-[hsl(var(--muted-foreground))]">Nearby Incidents</div>
                    <div className="text-2xl font-bold mt-2">{(tomtomIncidents?.length ?? 0)}</div>
                  </div>
                  {trafficInsights && trafficInsights[0] && (
                    <div className="mt-6 rounded-lg bg-emerald-600 text-white p-4">
                      <div className="bg-white text-black rounded-md p-3">
                        <p className="text-sm italic">{trafficInsights[0].suggestion}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-8">
            ...
            <Card className="p-8">
              <h3 className="text-2xl font-bold mb-6 flex items-center gap-2"><IconCalendar size={24} />5-Day Forecast</h3>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {weatherData.daily.temperature2mMax.slice(0, 5).map((maxTemp, idx) => (
                  <div key={idx} className="p-4 rounded-lg bg-[hsl(var(--card))]/50 border border-[hsl(var(--border))] text-center space-y-2">
                    <p className="text-sm font-medium">{new Date(Date.now() + idx * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { weekday: 'short' })}</p>
